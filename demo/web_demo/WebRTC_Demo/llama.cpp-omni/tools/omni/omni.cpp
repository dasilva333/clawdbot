...
static bool send_python_t2w_command(struct omni_context * ctx_omni, const std::string& cmd_json, std::string& response) {
    ...
    // 发送命令
    fprintf(stderr, "[CPP-DEBUG] Sending: %s\n", cmd_json.c_str()); fprintf(ctx_omni->python_t2w_stdin, "%s\n", cmd_json.c_str()); fflush(ctx_omni->python_t2w_stdin);
    fflush(ctx_omni->python_t2w_stdin);
...
bool omni_inject_text(struct omni_context * ctx_omni, std::string text) {
    // 1. Ensure system prompt is initialized
    if (!ctx_omni->system_prompt_initialized) {
        print_with_timestamp("omni_inject_text: initializing system prompt first\n");
        // Reuse stream_prefill(index=0) logic to setup system prompt & ref audio
        if (!stream_prefill(ctx_omni, "", "", 0, 0)) { // Assuming max_slice_nums=0
            LOG_ERR("%s: failed to initialize system prompt\n", __func__);
            return false;
        }
    }
