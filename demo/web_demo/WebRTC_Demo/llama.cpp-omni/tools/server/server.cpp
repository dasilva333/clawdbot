            ...
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                            ...
                            if (slot.n_past > 0 && slot.n_past < (int) slot.prompt.tokens.size()) {
                                ...
                                // this is useful for debugging prompt caching
                                {
                                    ...
                                    const int np1 = std::min<int>(
                                        slot.n_past + 6, std::min(slot.prompt.tokens.size(), slot.task->tokens.size()));
                    ...
                    if (lora_all_alora(slot.lora) && slot.alora_invocation_start - 1 > slot.n_past) {
                        SLT_DBG(slot,
                                "processing pre-alora tokens without the adapter (n_past = %d, alora_invocation_start "
                                "= %d)\n",
                                slot.n_past, slot.alora_invocation_start);
...
int main(int argc, char ** argv) {
        ...
        json all_metrics_def = json{
            { "counter",
             { { { "name", "prompt_tokens_total" },
                ...
                { { "name", "n_past_max" }, { "help", "Largest observed n_past." }, { "value", res_task->n_past_max } },
    ...
    // impl: prefill
    const auto handle_stream_prefill_impl = [&ctx_server, &res_ok, &res_error](const json &        data,
    ...
    // wrapper: prefill
    const auto handle_stream_prefill = [&handle_stream_prefill_impl](const httplib::Request & req,
                                                                     httplib::Response &      res) {
    ...
    // impl: decode
    const auto handle_stream_decode_impl = [&ctx_server, &res_ok, &res_error](const json &        data,
                                                                              httplib::Response & res) -> void {
        ...
        std::string debug_dir = data.contains("debug_dir") && data.at("debug_dir").is_string() ?
    ...
    const auto handle_stream_decode = [&handle_stream_decode_impl](const httplib::Request & req,
                                                                   httplib::Response &      res) {
    ...
    // impl: omni_init
    const auto handle_stream_omni_init_impl = [&ctx_server, &res_ok, &res_error, &params](
                                                  const json & data, httplib::Response & res) -> void {
        ...
        if (data.contains("msg_type") && data.at("msg_type").is_number_integer()) {
            ...
            res_error(res, format_error_response(
                               "\"msg_type\" or \"media_type\" must be provided as integer (1=audio, 2=omni)",
                               ERROR_TYPE_INVALID_REQUEST));
            return;
        }
        ...
        if (vision_backend == "coreml") {
            ...
            std::string vision_coreml =
                json_value(data, "vision_coreml_model_path",
        ...
        json ack = {
            ...
            { "use_tts",          use_tts                                                                                 },
            { "voice_audio_used", data.contains("voice_audio") && data.at("voice_audio").is_string() &&
                                      !data.at("voice_audio").get<std::string>().empty() }
        };
        res_ok(res, ack);
    };

    const auto handle_stream_omni_init = [&handle_stream_omni_init_impl](const httplib::Request & req,
                                                                         httplib::Response &      res) {
    ...
    const auto handle_tts_inject_text_impl = [&ctx_server, &res_ok, &res_error](const json &        data,
                                                                                httplib::Response & res) -> void {
        if (!data.contains("text") || !data.at("text").is_string()) {
            res_error(res, format_error_response("\"text\" must be provided as string", ERROR_TYPE_INVALID_REQUEST));
            return;
        }
        std::string text = data.at("text");
        ...
        if (!omni_inject_text(ctx_server.octx, text)) {
            res_error(res, format_error_response("omni_inject_text failed", ERROR_TYPE_SERVER));
            return;
        }
    ...
    const auto handle_tts_inject_text = [&handle_tts_inject_text_impl](const httplib::Request & req,
                                                                       httplib::Response &      res) {
        SRV_INF("%s: handle_tts_inject_text\n", __func__);
        json body = json::parse(req.body);
        handle_tts_inject_text_impl(body, res);
    };

    // ==================== 打断 API ====================
    // impl: break - 中断当前生成，用于双工模式下用户打断
    const auto handle_stream_break_impl = [&ctx_server, &res_ok, &res_error](const json &        data,
                                                                             httplib::Response & res) -> void {
        // Expected body fields:
    ...
    const auto handle_stream_break = [&handle_stream_break_impl](const httplib::Request & req,
                                                                 httplib::Response &      res) {
    ...
    const auto handle_stream_reset_impl = [&ctx_server, &res_ok, &res_error](const json &        data,
                                                                             httplib::Response & res) -> void {
        // Expected body fields:
    ...
    const auto handle_stream_reset = [&handle_stream_reset_impl](const httplib::Request & req,
                                                                 httplib::Response &      res) {
    ...
    const auto handle_stream_update_session_config_impl = [&ctx_server, &res_ok, &res_error](
                                                              const json & data, httplib::Response & res) -> void {
        // Expected body fields:
    ...
    const auto handle_stream_update_session_config = [&handle_stream_update_session_config_impl](
