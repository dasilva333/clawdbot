import { joinVoiceChannel, createAudioPlayer, createAudioResource, AudioPlayerStatus, VoiceConnectionStatus, entersState, StreamType, EndBehaviorType, } from "@discordjs/voice";
import { PassThrough } from "node:stream";
import fs from "node:fs";
import prism from "prism-media";
export class VoiceManager {
    sessions = new Map();
    pcmBuffers = new Map(); // guildId -> Buffer
    prefillQueue = new Map(); // guildId -> Last prefill promise
    logger;
    onAudio;
    onSpeechEnd;
    constructor(logger) {
    ...
    async join(guildId, channelId, adapterCreator) {
        ...
        const activeSession = { connection, player, channelId, guildId };
    ...
    playStream(guildId, stream, isRaw = false) {
        ...
        try {
            ...
            args.push("-i", "-", "-f", "s16le", "-ar", "48000", "-ac", "2");
